...
export class DynamicStructureValidator {
  ...
  discoverModules() {
    ...
    return readdirSync(this.corePath).filter((item) => {
      const itemPath = join(this.corePath, item);
      // Excluir carpetas no-módulo y solo incluir directorios
      return (
        !TESTING_CONFIG.dynamicModules.excludeFolders.includes(item) &&
  ...
  validateCoreExists() {
    const exists =
      existsSync(this.corePath) && statSync(this.corePath).isDirectory();
  ...
  validateModuleStructure(moduleName) {
    ...
    const foundRequiredCount =
      moduleConfig.requiredFolders.length - missingFolders.length;
    const minimumRequired =
      TESTING_CONFIG.dynamicModules.minimumRequiredFolders;
    ...
    result.structure.components = this.validateComponentsStructure(
      moduleName,
      modulePath
    );
    ...
    if (
      !result.structure.components.valid &&
      foundFolders.includes("components")
    ) {
      result.valid = false;
    }
  ...
  validateComponentsStructure(moduleName, modulePath) {
    ...
    const componentConfig =
      EXPECTED_STRUCTURE.core.modules.componentsSubfolders;
    ...
    if (!existsSync(componentsPath)) {
      result.valid = false;
      result.errors.push(
        `Carpeta "components" no encontrada en módulo ${moduleName}`
      );
      return result;
    }
    ...
    // Validar subcarpetas requeridas según la configuración
    if (componentConfig && componentConfig.requiredSubfolders) {
      const missingRequired = componentConfig.requiredSubfolders.filter(
        (folder) => !foundSubfolders.includes(folder)
      );

      if (missingRequired.length > 0) {
        result.valid = false;
        result.errors.push(
          `Faltan subcarpetas requeridas en components/: ${missingRequired.join(
            ", "
          )}`
        );
      }
    ...
    if (TESTING_CONFIG.dynamicModules.requireComponentsSubfolders) {
      const minSubfolders =
        TESTING_CONFIG.dynamicModules.minimumComponentsSubfolders;
    ...
    if (foundSubfolders.length > 0 && !foundSubfolders.includes("common")) {
      result.warnings.push(
        `Se recomienda tener una carpeta "common" en components/`
      );
    }

    // Verificar archivo index
    const indexExists = EXPECTED_STRUCTURE.core.modules.indexFiles.some(
      (indexFile) => {
  ...
  generateSummary() {
    const validModules = Object.values(this.results.modules.validations).filter(
      (module) => module.valid
    ).length;
  ...
  printReport() {
    ...
    if (shared.foundFolders) {
      console.log(
        `   📂 Carpetas encontradas: ${shared.foundFolders.join(", ")}`
      );
    }
    ...
    Object.values(modules.validations).forEach((module) => {
      ...
      if (module.structure.components) {
        ...
        console.log(
          `      ${compStatus} Components: ${module.structure.components.foundSubfolders.length} subcarpetas`
        );
        if (module.structure.components.foundSubfolders.length > 0) {
          console.log(
            `         📂 ${module.structure.components.foundSubfolders.join(
              ", "
            )}`
          );
        }
    ...
    console.log(
      `\n🎯 RESULTADO FINAL: ${
        summary.overallValid ? "✅ VÁLIDO" : "❌ INVÁLIDO"
      }`
    );
